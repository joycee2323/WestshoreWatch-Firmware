 ...
 * misses real traffic. ONE channel is visited per peek, round-robin, so a
 * full-band sweep now spans 25 peeks instead of 5. Per-peek dwell/interval
 * (FIVE_GHZ_DWELL_MS / FIVE_GHZ_INTERVAL_MS) are deliberately UNCHANGED —
 * this is the "naive widen" the investigation scoped, not the smarter
 * prioritized-sweep restructuring it also discussed. That means worst-case
 * cold-search time to first lock on an unknown 5 GHz signal goes from ~25s
 * (5 channels) to ~125s (25 channels) — a straight-line 5x, since round-robin
 * period scales linearly with list length while dwell/interval don't. This
 * is a deliberate, documented tradeoff, not an oversight; revisit if that
