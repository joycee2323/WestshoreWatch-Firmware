 ...
 * radiating the cached data buffer (ported from the production tree's
 * round-3 fix e981590; its absence here is how a node radiated one frozen
 * Pack for 30+ min on 2026-09-22). Set + start with a Pack carrying a Basic
 * ID with the same "WSW-<device_id>" idle content as handle 0 (see
 * encode_bridge_beacon() above) and msg_count=1; the app's upload guard
 * drops anything with that UAS_ID prefix, same semantics as handle 0. */
...
static bool maybe_evict_slot(drone_slot_t *s, TickType_t now)
{
    ...
    const rp_limits_t lim = {
        .land_ticks   = pdMS_TO_TICKS(5000),
        .silent_ticks = pdMS_TO_TICKS(15000),
...
static void relay_task(void *arg)
{
    ...
    while (s_running) {
        ...
        if (live == 0) {
            ...
            /* Overwrite handle 1 — a stop alone leaves the controller
             * radiating the last drone's Pack (see
             * advertise_pack_idle_placeholder()). */
            rp_radio_note_write(&s_pack_health, advertise_pack_idle_placeholder(), now);
