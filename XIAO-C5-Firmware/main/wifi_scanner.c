 ...
 * 2.4 GHz primary dwell, so the 2.4 GHz/BLE (DJI) radio budget is untouched.
 * The lock is channel-level (keyed on the 5 GHz channel, not on any drone/MAC).
 *
...
static void promiscuous_cb(void *buf, wifi_promiscuous_pkt_type_t type)
{
    ...
    if (ret >= 0) {
         ...
         * hop task reads these to hold/release the 5 GHz peek target. */
        if (rx_ch > 14) {
