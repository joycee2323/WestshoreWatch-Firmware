...
- **Adaptive channel-lock:** once a peek decodes a 5 GHz ODID frame, the peek
  slot biases toward that channel instead of continuing the even round-robin,
  so a loitering Skydio gets held rather than only re-caught every other
  peek. `FIVE_GHZ_RELEASE_MISSES` (default **5**) consecutive misses on the
  locked channel release the lock back to round-robin;
  `FIVE_GHZ_LOCK_ACTIVE_PER_SWEEP` (default **4**) spends one peek in every
  5 sweeping the other channel(s) while locked, so a channel-hopping drone is
